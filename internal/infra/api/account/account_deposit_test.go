...
func TestHandleAccountDeposit(t *testing.T) {
	...
	fmt.Println("Testing with URL:", fmt.Sprintf("/api/v1/accounts/%s/deposit", accountID))
	req := httptest.NewRequest("POST", "/api/v1/accounts/{account_id}/deposit", bytes.NewReader(body))
	...
	handler := HandleAccountDeposit(mockAccountRepository)
	...
	expectedMessage := float64(1050)
	assert.Equal(t, expectedMessage, response["balance"], "Expected response balance to be: 1050")
}
